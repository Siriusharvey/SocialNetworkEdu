...
<div style='margin-bottom: 6px;'>
  ...
  <td style='text-align: right;'>
    ...
    <a id="last" href="javascript:void(0);" onclick="nextpic(0,{$media[$prev_media_id].media_id},'{$page_owner->user_info.user_username}',true);">{lang_print id=1000146}</a>
    
    &nbsp;&nbsp;&nbsp;
    {capture assign="next_media_id"}{$media_keys.$next_index}{/capture}
    <a id="next" href="javascript:void(0);" onclick="nextpic(0,{$media[$next_media_id].media_id},'{$page_owner->user_info.user_username}',true);">{lang_print id=1000147}</a>
...
{* SHOW IMAGE *}
<div class='media'><div  id='loading' style='height:32px; z-index:1000; text-align:center; margin-bottom:-10px; position:absolute; width:500px; display:none'><img src='./images/icons/loading2.gif' border='0'></div>
  <table cellpadding='0' cellspacing='0' align='center'>
  <tr>
  <td style='text-align: center;'>

    {* CREATE WRAPPER DIV *}
    <div id='media_photo_div' align='left' class='media_photo_div' style='width:{$media_info.media_width}px;height:{$media_info.media_height}px;'>

      {* DISPLAY IMAGE *}
      <div id='photo_div_{$media_info.media_id}'  style='position:absolute; opacity: 1;'>
    ...
    <div id="media_caption" class='media_caption' style='width: {if $media_info.media_width > 300}{$media_info.media_width}{else}300{/if}px;'>
      <div id='title' class='media_title' {if $media_info.media_title == ""}style='display:none;'{/if}>{if $media_info.media_title != ""}{$media_info.media_title}{/if}</div>
      ...
      <div id='from'>{if $media_info.user_id != 0}{lang_sprintf id=1216 1=$parent_link 2=$media_info.media_parent_title 3=$url->url_create("profile", $media_info.user->user_info.user_username) 4=$media_info.user->user_displayname}{else}{lang_sprintf id=1217 1=$parent_link 2=$media_info.media_parent_title}{/if}</div>
      ...
      <span id="posted_date">
        ...
        <a  id="report_content" {if !$user->user_exists}style='display:none;'{/if} href="javascript:TB_show('{lang_print id=1221}', 'user_report.php?return_url={$url->url_current()|escape:url}&TB_iframe=true&height=300&width=450', '', './images/trans.gif');">{lang_print id=1221}</a>
      </div>
    </div>
  </td>
  </tr>
...
<div style='display: none;' id='sharethis'>
  ...
  <textarea id="direct_link" readonly='readonly' onClick='this.select()' class='text' rows='2' cols='30' style='width: 95%; font-size: 7pt;'>{$url->url_base}{$media_info.media_dir|replace:"./":""}{$media_info.media_id}.{$media_info.media_ext}</textarea>
  <div style='margin: 10px 0px 10px 0px; font-weight: bold;'>{lang_print id=1224}</div>
  <textarea id="embedded_image" readonly='readonly' onClick='this.select()' class='text' rows='2' cols='30' style='width: 95%; font-size: 7pt;'><a href='{$url->url_base}{$media_info.media_dir|replace:"./":""}{$media_info.media_id}.{$media_info.media_ext}'><img src='{$url->url_base}{$media_info.media_dir|replace:"./":""}{$media_info.media_id}.{$media_info.media_ext}' border='0'></a></textarea>
  <div style='margin: 10px 0px 10px 0px; font-weight: bold;'>{lang_print id=1225}</div>
  <textarea id="text_link" readonly='readonly' onClick='this.select()' class='text' rows='2' cols='30' style='width: 95%; font-size: 7pt;'><a href='{$url->url_base}{$media_info.media_dir|replace:"./":""}{$media_info.media_id}.{$media_info.media_ext}'>{if $media_info.media_title != ""}{$media_info.media_title}{else}{lang_print id=589}{/if}</a></textarea>
  <div style='margin: 10px 0px 10px 0px; font-weight: bold;'>{lang_print id=1226}</div>
  <textarea id="ubb_code" readonly='readonly' onClick='this.select()' class='text' rows='2' cols='30' style='width: 95%; font-size: 7pt;'>[url={$url->url_base}{$media_info.media_dir|replace:"./":""}{$media_info.media_id}.{$media_info.media_ext}][img]{$url->url_base}{$media_info.media_dir|replace:"./":""}{$media_info.media_id}.{$media_info.media_ext}[/img][/url]</textarea>
...
<!--
 ...
 //   myFx = new Fx.Scroll('album_carousel');
   ...
   // myFx.set(position.x, position.y);
    updateCarousel('next', 0, {/literal}{$page_owner->user_info.user_id}{literal}, true )     ;        
    block_val = false;  
  });
