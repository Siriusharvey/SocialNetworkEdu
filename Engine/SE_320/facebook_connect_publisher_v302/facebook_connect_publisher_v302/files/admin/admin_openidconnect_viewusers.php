<?php
$page = "admin_openidconnect_viewusers";
...
if(isset($_POST['f_service'])) { $f_service = $_POST['f_service']; } elseif(isset($_GET['f_service'])) { $f_service = $_GET['f_service']; } else { $f_service = ""; }
...
if($s == "i") {
  ...
  $sort = "openidservice_displayname";
  $svc = "svcd";
} elseif($s == "svcd") {
  $sort = "openidservice_displayname DESC";
  $svc = "svc";
} else {
  $sort = "user_id DESC";
  $i = "i";
...
$user_query = "SELECT user_id, user_username, user_fname, user_lname, user_email, user_enabled, user_verified, user_signupdate, user_level_id, user_subnet_id, se_semods_usersopenid.*, se_semods_openidservices.* FROM ";

$user_query .= "se_semods_usersopenid JOIN se_users ON se_semods_usersopenid.openid_user_id = se_users.user_id JOIN  se_semods_openidservices ON se_semods_usersopenid.openid_service_id = se_semods_openidservices.openidservice_id";
...
if($f_service != "") { $where_clause[] = "openidservice_id='$f_service'"; }
...
$openid_services = semods::db_query_assoc_all("SELECT * FROM se_semods_openidservices ORDER BY openidservice_displayname ASC");
...
$smarty->assign('f_service', $f_service);
