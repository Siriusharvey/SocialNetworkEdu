...
{* $Id: profile.tpl 130 2009-03-21 23:36:57Z john $ *}
...
{* BEGIN LEFT COLUMN *}
  ...
  {if $owner->user_info.user_id != $user->user_info.user_id}
    ...
    {if $user->user_exists != 0}
      <tr><td class='profile_menu1' nowrap='nowrap'><a href="javascript:TB_show('{lang_print id=857}', 'user_report.php?return_url={$url->url_current()}&TB_iframe=true&height=300&width=450', '', './images/trans.gif');"><img src='./images/icons/report16.gif' class='icon' border='0'>{lang_print id=840}</a></td></tr>
      {assign var='showmenu' value='1'}
    {/if}
  ...
  {* BEGIN RIGHT COLUMN *}
	...
	{if $sms_disable =="0" && $userid !="" && $sms_disable_mobile !="" && $to_sms_disable_mobile !=""}
<td valign='bottom'><table cellpadding='0' cellspacing='0'><td class='profile_tab{if $v == 'sms'}2{/if}' id='profile_tabs_sms' onMouseUp="this.blur()"><a href='javascript:void(0);' onMouseDown="loadProfileTab('sms');" onMouseUp="this.blur()">{lang_print id=5000001}</a></td></tr></table></td>	{/if}
    ...
    {if $allowed_to_comment != 0 || $total_comments != 0}
      ...
      <div id='profile_comments'{if $v != 'comments'} style='display: none;'{/if}>
        ...
        {lang_javascript ids=39,155,175,182,183,184,185,187,784,787,829,830,831,832,833,834,835,854,856,891,1025,1026,1032,1034,1071,5000001}
...
var longitud=caracteres - entradaObj.value.length;
  ...
  salidaObj.innerHTML = texto.replace("CHAR",longitud);
      ...
      {* SHOW SMS TAB *}
      <div id='profile_sms'{if $v != 'sms'} style='display: none;'{/if}>
        
            <div class='profile_headline'>SMS</div>
          
        {* SMS *}
		<form method="post" action="profile.php?user={$owner->user_info.user_username}&v=sms">
       	<div>
			{if $sms_sent != ""}<div align="center" class='error'>{$sms_sent}</div>{/if}
			<textarea name="sms_msg" cols="50" rows="6"  id="eBann" maxlength="100" size="60" onKeyUp="Contar('eBann','sBann','CHAR characters left.',160);"></textarea>
			<DIV id="sBann" class="o" align="center">160 characters left.</DIV>
