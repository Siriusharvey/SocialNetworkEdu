...
$sms_dis = $database->database_query("select plugin_disabled  from se_plugins where plugin_id ='1'");
$sms_disable=$database->database_fetch_assoc($sms_dis);
$sms_disable=$sms_disable[plugin_disabled];
$smarty->assign("sms_disable",$sms_disable);

$sms_mobile = $database->database_query("select mobile_no from se_user_smssetting where user_id ='$userid'");
$sms_disable_mobile=$database->database_fetch_assoc($sms_mobile);
$sms_disable_mobile=$sms_disable_mobile[mobile_no];
$smarty->assign("sms_disable_mobile",$sms_disable_mobile);



$resource3 = $database->database_query("select user_id from se_users where user_username='".$_GET[user]."'");
$result3=$database->database_fetch_assoc($resource3);
$result3_id=$result3[user_id];



$user_mobile =$_GET[user];
$to_sms_mobile = $database->database_query("select mobile_no from se_user_smssetting where  user_id ='$result3_id' and user_id !='$userid'");
$to_sms_disable_mobile=$database->database_fetch_assoc($to_sms_mobile);
$to_sms_disable_mobile=$to_sms_disable_mobile[mobile_no];
$smarty->assign("to_sms_disable_mobile",$to_sms_disable_mobile);
...
if($v != "profile" && $v != "friends" && $v != "comments" && $v != "sms" && !array_key_exists($v, $global_plugins)) { $v = "profile"; }
...
if($v== "sms" && $user->user_info['user_id'] == $owner->user_info['user_id']){
$username=$user->user_info['user_id'];
 $msms=$_POST['member_sms'];
 $psms=$_POST['admin_sms'];
 $sql1=" update se_users set member_sms='".$msms."',admin_sms='".$psms."' where user_id='".$username."'";
...
$smsexp_resource = $database->database_query("select ssms_credits from se_user_smssetting where user_id='$userid'");
$smsexp_result = $database->database_fetch_assoc($smsexp_resource);

if($smsexp_result[ssms_credits] == "")
{
	header("Location: sms_settings.php");
	exit;
}
elseif($smsexp_result[ssms_credits] <= 0)
{
	header("Location: buy_credits.php");
	exit;
}

$resource = $database->database_query("select mobile_no,ssms_credits from se_user_smssetting where user_id='$userid'");
$result1=$database->database_fetch_assoc($resource);

$resource3 = $database->database_query("select user_id from se_users where user_username='".$_GET[user]."'");
$result3=$database->database_fetch_assoc($resource3);
$result3_id=$result3[user_id];

$resource2 = $database->database_query("select mobile_no from se_user_smssetting where user_id='$result3_id'");
$result2=$database->database_fetch_assoc($resource2);


	$msi = $result2['mobile_no'];
	$txt=$_POST['sms_msg']."\n Sender Phone Number:".$result1['mobile_no'];
	$dt=$result1['mobile_no'];
	$tudei = date('Y-m-d');
	...
	$smarty->assign('sms_sent', 'Sent Successfully!');
	}
	else
		$smarty->assign('sms_sent', 'Send Failed!');
...
$iduser=$user->user_info['user_id'];
 $sql_sms="select member_sms,admin_sms from se_users where user_id='".$iduser."'";
$row_sms=mysql_query($sql_sms);
$res_sms=mysql_fetch_array($row_sms);
 $member_sms=$res_sms['member_sms'];
 $admin_sms=$res_sms['admin_sms'];
...
$smarty->assign('member_sms', $member_sms);
$smarty->assign('admin_sms', $admin_sms);
